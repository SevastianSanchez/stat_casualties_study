...
# TWFE Direct Effects
coefs_fe_direct <- summary(twfe_dir_dynamic, vcov = vcovHC(twfe_dir_dynamic, cluster = "group", type = "HC1"))$coefficients
coef_df_fe_direct <- data.frame(
  model = "TWFE_Direct",
  term = rownames(coefs_fe_direct),
  estimate = coefs_fe_direct[, "Estimate"],
  std.error = coefs_fe_direct[, "Std. Error"],
  ci_low = coefs_fe_direct[, "Estimate"] - 1.96 * coefs_fe_direct[, "Std. Error"],
  ci_high = coefs_fe_direct[, "Estimate"] + 1.96 * coefs_fe_direct[, "Std. Error"]
)

# TWFE Mediation Effects
coefs_fe_med <- summary(twfe_med_dynamic, vcov = vcovHC(twfe_med_dynamic, cluster = "group", type = "HC1"))$coefficients
coef_df_fe_med <- data.frame(
  model = "TWFE_Mediation",
  term = rownames(coefs_fe_med),
  estimate = coefs_fe_med[, "Estimate"],
  std.error = coefs_fe_med[, "Std. Error"],
  ci_low = coefs_fe_med[, "Estimate"] - 1.96 * coefs_fe_med[, "Std. Error"],
  ci_high = coefs_fe_med[, "Estimate"] + 1.96 * coefs_fe_med[, "Std. Error"]
)

# Combine all into one dataframe
stage2_model_results <- rbind(coef_df_fd_direct, coef_df_fd_med, coef_df_fe_direct, coef_df_fe_med)
...
# Save the plot
ggsave("panel_regression/figures/stage2/error_bar_twtwfe_med_dynamic.png", ebar_fe, width = 10, height = 6)
```
