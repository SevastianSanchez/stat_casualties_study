...
Basing analysis of dataset sensitivity on FE model of the second order (twfe_spi_di_dymanic)
...
ols_spi_di_dymanic <- plm(
  formula = spi_comp ~ di_score + plm::lag(di_score, 1) + plm::lag(di_score, 2)
  + log_gdppc 
  + factor(year), 
  model = "pooling", 
  data = panel_data)
summary(ols_spi_di_dymanic, vcov = vcovHC(ols_spi_di_dymanic, cluster = "group", type = "HC1"))
```

## Checking which countries are dropped due to missingness
```{r}
# rows actually used in the model
model_rows <- rownames(model.frame(ols_spi_di_dymanic))
...
summary(fd_spi_di_dymanic, vcov = vcovHC(fd_spi_di_dymanic, cluster = "group", type = "HC1"))

# joint significance 
# names(coef(fd_spi_di_dymanic)) # ensure correct names 
linearHypothesis(
  fd_spi_di_dymanic,
  c("di_score = 0", "plm::lag(di_score, 1) = 0", "plm::lag(di_score, 2) = 0"),
  vcov. = vcovHC(fd_spi_di_dymanic, cluster = "group", type = "HC1"), 
  test = "F"
)
...
```{r}
# Contemporaneous Effects: SPI ~ DI [REPORTED]
twfe_spi_di_current <- plm(
  formula = spi_comp ~ di_score 
  + log_gdppc 
  + factor(year),
  data = panel_data,
  model = "within")
summary(twfe_spi_di_current, vcov = vcovHC(twfe_spi_di_current, cluster = "group", type = "HC1"))

# Lag2: SPI ~ DI [REPORTED]
twfe_spi_di_dymanic <- plm(
  formula = spi_comp ~ di_score 
  + plm::lag(di_score, 1) 
  + plm::lag(di_score, 2) 
  + log_gdppc 
  ...
  data = panel_data,
  model = "within")
summary(twfe_spi_di_dymanic, vcov = vcovHC(twfe_spi_di_dymanic, cluster = "group", type = "HC1"))

# joint significance: is there an effect between these three variables? 
linearHypothesis(
  twfe_spi_di_dymanic,
  c("di_score = 0", "plm::lag(di_score, 1) = 0", "plm::lag(di_score, 2) = 0"),
  vcov. = vcovHC(twfe_spi_di_dymanic, cluster = "group", type = "HC1"), 
  test = "F"
)
...
```{r}
# CI for all variables (dymanic model)
robust_results_fd <- coeftest(fd_spi_di_dymanic, vcov = vcovHC(fd_spi_di_dymanic, cluster = "group", type = "HC1"))

# Create data frame for plotting
coef_fd_df <- data.frame(
  model = "FD",
  term = rownames(robust_results_fd),
  estimate = robust_results_fd[, "Estimate"],
  std_error = robust_results_fd[, "Std. Error"],
  ci_low = robust_results_fd[, "Estimate"] - 1.96 * robust_results_fd[, "Std. Error"],
  ci_high = robust_results_fd[, "Estimate"] + 1.96 * robust_results_fd[, "Std. Error"]
)

# CI for all variables (dymanic model)
robust_results_fe <- coeftest(twfe_spi_di_dymanic, vcov = vcovHC(twfe_spi_di_dymanic, cluster = "group", type = "HC1"))

# Create data frame for plotting
coef_twfe_df <- data.frame(
  model = "TWFE",
  term = rownames(robust_results_fe),
  estimate = robust_results_fe[, "Estimate"],
  std_error = robust_results_fe[, "Std. Error"],
  ci_low = robust_results_fe[, "Estimate"] - 1.96 * robust_results_fe[, "Std. Error"],
  ci_high = robust_results_fe[, "Estimate"] + 1.96 * robust_results_fe[, "Std. Error"]
)

# combining 
stage1_models_results <- rbind(coef_fd_df, coef_twfe_df)

# view dataframes
head(stage1_models_results)

# save dataframes for later use
write.csv(stage1_models_results, "panel_regression/model_results/stage1_model_results.csv", 
          row.names = FALSE)
```

## First Difference CI Results & Error Bar Visualization (dymanic)
```{r}
# filter to only the first four rows (variables of interest)
sub_model <- stage1_models_results %>%
  filter(model == "FD", 
         term %in% c("di_score", 
                     "plm::lag(di_score, 1)", 
                     "plm::lag(di_score, 2)", 
...
# Save the plot
#ggsave("panel_regression/figures/stage1/error_bar_stage1_fd.png", ebar_fe, width = 10, height = 6)
```

## Two-Way Fixed Effects Error Bar Visualization 
```{r}
# filter to only the first four rows (variables of interest)
sub_model <- stage1_models_results %>%
  filter(model == "TWFE", 
...
# Save the plot
#ggsave("panel_regression/figures/stage1/error_bar_stage1_twfe.png", ebar_fe, width = 10, height = 6)
```
...
combined_stage1_ebar_plot

# Save the combined plot
ggsave("panel_regression/figures/stage1/combined_error_bars_stage1.png", 
       combined_stage1_ebar_plot, width = 12, height = 8, dpi = 300)

```

## (Dynamic) Effect Sizes Visualization [REVISED - AI]
```{r}
# Use existing dataframes created earlier:  coef_fd_plot_df and coef_twfe_plot_df
...
fd_di_data <- stage1_models_results %>%
  filter(model == "FD",
    term %in% c("di_score", "plm::lag(di_score, 1)", "plm::lag(di_score, 2)")) %>% 
  arrange(desc(term))

twfe_di_data <- stage1_models_results %>%
  filter(model == "TWFE",
    term %in% c("di_score", "plm::lag(di_score, 1)", "plm::lag(di_score, 2)")) %>% 
  arrange(desc(term))
...
twfe_coef <- twfe_di_data$estimate
twfe_se <- twfe_di_data$std_error
...
# Save normally
ggsave("panel_regression/figures/stage1/combined_dynamic_effects_s1_color.png", 
...
# Save bw version 
ggsave("panel_regression/figures/stage1/combined_dynamic_effects_s1_bw.png", 
       plot = combined_plot_bw, width = 12, height = 6)
```
...
stargazer(
  fd_spi_di_current, fd_spi_di_dymanic, twfe_spi_di_current, twfe_spi_di_dymanic,
  se = list(
    coeftest(fd_spi_di_current, vcov = vcovHC(fd_spi_di_current, cluster = "group", type = "HC1"))[, 2],
    coeftest(fd_spi_di_dymanic, vcov = vcovHC(fd_spi_di_dymanic, cluster = "group", type = "HC1"))[, 2],
    coeftest(twfe_spi_di_current, vcov = vcovHC(twfe_spi_di_current, cluster = "group", type = "HC1"))[, 2],
    coeftest(twfe_spi_di_dymanic, vcov = vcovHC(twfe_spi_di_dymanic, cluster = "group", type = "HC1"))[, 2]
  ),
  type = "html",
  ...
  omit = "factor\\(year\\)",
  model.names = FALSE,
  out = "panel_regression/figures/stage1/stage1_mods.html"
)
...
pbgtest(twfe_spi_di_dymanic) # Panel
pwartest(twfe_spi_di_dymanic) # FE [significant]
pwfdtest(fd_spi_di_dymanic) # FD [Not significant: NO EVIDENCE OF AUTOCORRELATION]
```
...
bptest(twfe_spi_di_dymanic, studentize = TRUE) # [significant]
bptest(fd_spi_di_dymanic, studentize = TRUE) # [significant]
```
